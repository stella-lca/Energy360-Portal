...
const handleToken = async function (authCode, tokenData) {
  ...
  if (!access_token) {
    createLogItem(
      true,
      "Token Management",
      "Token API Don't have valid contents"
    );
    console.log(
      "handleToken =>",
      "Token Management",
...
exports.notifyCallback = async function (req, res) {
  try {
    ...
    createLogItem(
      true,
      "Utility API Response",
      "Got the Utility Notify Request",
    ...
    //   );
      ...
      createLogItem(
        true,
        "GreenConnect - Utility API Response",
        "Received the utility callback, content is empty",
        JSON.stringify(fileUrls)
      );
      res.status(200).send("ok");
    }
