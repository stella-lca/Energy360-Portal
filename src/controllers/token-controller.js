...
const accessTokenGenerator = async function(data, headers) {
	try {
		const newToken = (
			await axios.post("https://apit.coned.com/gbc/v1/oauth/v1/token", data, {
				headers
			})
		).data;
...
module.exports.authenticateToken = async function(req, res) {
	...
	try {
		...
		if (tokens.length === 0) {
			...
			tokenData = (
				await updateToken({ expires_in, expiry_date, scope, authorizationURI })
			).data;
		}
		//save in session
